...
// Test endpoint - créer une application de test avec token valide
app.get("/api/test-create-application", (req, res) => {
  try {
    const testAppId = "test-" + Date.now();
    const testToken = generateToken();
    
    // Créer l'application
    applicationsData[testAppId] = {
      ...
      technicalFormCompleted: false,
      technicalFormSubmittedAt: null,
    };
    
    // Créer le token
    tokensData[testToken] = {
    ...
    const technicalFormUrl = `http://localhost:5173?technical-form=true&token=${testToken}&appId=${testAppId}`;
    ...
    res.json({
      ...
      token: testToken,
      applicationId: testAppId,
      instructions: "Clique sur le lien pour accéder au formulaire technique et ajouter ton Gmail/mot de passe"
    });
  } catch (error) {
    res.status(500).json({
